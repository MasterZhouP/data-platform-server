...
@RestControllerAdvice(assignableTypes = DatasourceAdminController.class)
public class DatasourceApiExceptionHandler {
    private static final Logger log = LoggerFactory.getLogger(DatasourceApiExceptionHandler.class);
    ...
    public ResponseEntity<AjaxResult> unavailable(DataAccessException error, HttpServletRequest request) {
        log.warn("数据源配置存储不可用 requestId={} type={}", requestId(request), error.getClass().getSimpleName());
    ...
    public ResponseEntity<AjaxResult> unexpected(Exception error, HttpServletRequest request) {
        log.error("数据源管理请求异常 requestId={} type={}", requestId(request), error.getClass().getSimpleName());
        return respond(request, 500, "CONFIGURATION_OPERATION_FAILED", "数据源操作失败，请根据请求标识联系管理员");
    }
