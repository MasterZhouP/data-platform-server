...
{
    @Test void serviceCredentialAuthenticatesOnlyDedicatedApiChainAndJwtCannotReplaceIt() throws Exception
    {
        try (var context = new AnnotationConfigWebApplicationContext())
        {
            ...
            mvc.perform(get("/integration/openapi/v1/status").header("X-Integration-Key", client.getKey()).header("X-Request-Id", id))
                    .andExpect(status().isOk());
            mvc.perform(get("/integration/reference/options").header("X-Integration-Key", client.getKey()).header("X-Request-Id", id))
