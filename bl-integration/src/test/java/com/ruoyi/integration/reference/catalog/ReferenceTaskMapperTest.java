...
class ReferenceTaskMapperTest {
    @Test void persistsAcrossSessionsWithBoundJsonAndAtomicVersionCheck() throws Exception {
        ...
        try (var connection = db.getConnection(); var statement = connection.createStatement()) {
            statement.execute("CREATE TABLE int_reference_task (task_code VARCHAR(100) PRIMARY KEY, task_name VARCHAR(100), enabled BOOLEAN, datasource_key VARCHAR(100), sql_text CLOB, metadata_json CLOB, metadata_version VARCHAR(100), create_time TIMESTAMP, update_time TIMESTAMP)");
        }
        ...
        try (var session = sessions.openSession(true)) {
            ...
            assertEquals(1, mapper.insert(new ReferenceTaskRow("DEMO", "示例", false, "u8", "SELECT 'DEMO' AS code", metadata, "1")));
        }
        try (var session = sessions.openSession(true)) {
            var mapper = session.getMapper(ReferenceTaskMapper.class);
            ...
            var changed = new ReferenceTaskRow("DEMO", "更新", true, "u8", "SELECT 'DEMO' AS code", "{}", "2");
